...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as `npm run verify:deterministic`, `npm run verify:browser`, Playwright, or the targeted tests. State the outcome of each check, not only the command name.
...
A subagent that opens a PR runs `interrogate`, `/skill:unslop`, and `/skill:no-comments`. It returns the URL and does not babysit. Return to the parent.
