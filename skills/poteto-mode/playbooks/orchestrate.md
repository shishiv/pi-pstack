...
- **Worker / verifier.** Always `worktree: true` unless the task needs this machine: `control-ui` or `control-cli` runtime verification (from `installed Pi control skills`); reading the active session transcript named by `$PI_SESSION_FILE`; simulators and local IDE state; auth that exists only here. Managed-worktree subagents cannot read the local store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the managed-worktree subagent's status in the Pi run status. Transcript mtime is not liveness.
