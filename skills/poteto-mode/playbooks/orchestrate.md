...
- Every dispatch and every continuation carries the standing orders verbatim.
...
- **Coordinator (this chat).** Frames, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit is bookkeeping the coordinator may do itself when local git is cheap. Follow [`../../../docs/delegation.md`](../../../docs/delegation.md) for every dispatch and completion. State reads and writes may go through `scripts/orch/orch.ts` at drain points when that CLI is available. The CLI never dispatches, waits, or wakes anything.
- **Sub-coordinator.** One per track only when the program exceeds what one coordinator can drain and the selected delegation capability supports nested delegation. Otherwise the coordinator owns the tracks directly. A sub-coordinator owns its track's units and boards, authors briefs, dispatches workers and verifiers, and rolls up aggregates at wave boundaries. It never forwards raw child reports. Cap in-flight children at what one drain can process rather than using blocking batches.
- **Worker / verifier.** Give writers isolated worktrees unless the task needs this machine for the project verification skill, the active session transcript named by `$PI_SESSION_FILE`, simulators, local IDE state, or local-only authentication. Briefs inline what an isolated child cannot read. Prefer fewer, broader workers; one writer per worktree or branch. Run a unit's verifier on a different model family when the host reports one as available.
...
FORBIDDEN    no direct stack-backend commands, no rebase, no force-push, no fixes outside scope,
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. A delegate with access to the store may reference the standing-orders file by path. Otherwise include the standing orders in the task.

A sub-coordinator brief adds its track boundary and unit list, dispatch budget, drain protocol, and rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
2. **Initialize the state.** Run `orch init` when the optional CLI is available. Otherwise create the canonical store files directly. Open the trail via the show-me-your-work skill, write the standing orders before any dispatch, and seed `frontier.json` from existing PRs.
...
- The frontier is a computed object, never narrative. Recompute it from the
  selected delivery adapter after every merge or stack mutation because
  GitHub base refs drift mid-restack. The adapter receipt carries the ordered
  PR list, branch names, head SHAs, a generation number, and the lowest
  unmerged PR. Resources do not keep a second stack state or invoke a backend.
- Exactly one delivery adapter writer owns topology per stack; record the
  holder in the standing orders. Restacks run in a managed worktree when the
  adapter requires one; a local restack at this scale takes the laptop down.
- Workers never rebase or mutate topology. Babysitters follow
  `playbooks/babysit.md`, one per stack, scoped to one immutable frontier
  generation; they report conflicts to the adapter owner rather than
  restacking.
...
- Never resume a delegate merely to check on it. Probe read-only through the ledger, `units.tsv`, repository state, pushed branches, and the status exposed by the selected delegation capability. Transcript mtime is not liveness.
...
- After a Pi restart, re-read the standing orders and `units.tsv`, inspect the selected host for surviving work, and reconcile by PR and branch rather than delegate id. Restart missing work from its stored brief plus current state, then drain. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
