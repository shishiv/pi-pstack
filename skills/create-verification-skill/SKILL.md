...
description: "Generate a project-local verification skill that drives your app the way a user does in any language, framework, or platform. Use for /skill:create-verification-skill, \"make a control skill for this repo\", or when a project has no scripted way to prove UI/CLI/service behavior."
...
- **Drive:** how can an agent interact with it programmatically? Existing harnesses come first. Look for Playwright/Cypress specs, expect scripts, PTY helpers, curl-able endpoints, or a debug port. Only then pick a generic recipe. Use browser/CDP for web and Electron, a tmux/PTY harness for CLI/TUI, or plain HTTP for services.
...
Write `.pi/skills/verify-<app>/SKILL.md` with YAML frontmatter. Set `name: verify-<app>` and add a `description` that names the app, the surface, and when to reach for it. Without frontmatter, the skill never registers. Include these sections, ground each in what the interview actually found, and leave no placeholders:

- **Launch:** the exact command that starts the app for verification, and how to tell it's ready (a log line, a port answering, a prompt). Include teardown. For a short-lived CLI or TUI there is no server to keep alive: launch means build the binary (or install deps) once, then start each drive in its own isolated PTY or tmux session.
- **Doctor:** one read-only check that answers "is this instance worth driving?" It confirms the process is up, the version or build is right, the port is owned by us, and auth is valid. An agent runs this first whenever anything looks off.
...
Run its own instructions end to end once. Launch it, run doctor, drive ONE mapped feature, capture evidence, and clean up. One feature is enough because the map lets later runs cover the rest. After cleanup, confirm the evidence still exists at the named location. A cleanup that eats the proof fails this step. Fix what fails. Run the generated cleanup after every failed iteration so broken attempts don't strand processes and ports. A generated skill that was never executed is a draft, not a deliverable.
