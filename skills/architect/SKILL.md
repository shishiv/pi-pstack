...
Use the configured `oracle`, `worker`, `scout`, and `reviewer` agents. Pass an explicit model only when `/subagents-models` confirms it is available and a cross-provider comparison is intentional.
