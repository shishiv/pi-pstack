...
Prefer configured Pi Slack actions. Use `BENNY_SLACK_BOT_TOKEN` only when the user configured it for a narrow missing capability such as editing this one status message. Never expose the token to a worker.
...
Read `references/control-adapter.md` and the completed map at `control.featureMapPath`, then invoke the skill named by `control.skill_name`.
