...
Benny has two Pi-native operations: `benny-triage` classifies one new top-level report and posts exactly one verdict in its original thread; `benny-reproduce` waits for a trusted triage marker, drives a configured control adapter, and requires two independent matching UI observations before verification or a bounded fix. A host scheduler may invoke them, but scheduling is optional.
