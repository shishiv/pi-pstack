...
export function delegationGuidance(environment: DelegationEnvironment): string {
  ...
  if (environment.kind === "herdr-cli") {
    return `${PARENT_DELEGATION_REQUIREMENT} Herdr CLI is verified in this managed pane. Prefer the host agents capability exposed through exec as tools.agents. When multiple independent workstreams use tools.agents through exec, create each agent with blocking:false. Do only useful work that does not depend on those agents. If results are still pending, end the turn. The host delivers completions automatically and resumes the session. Do not use watch, read, wait, or sleep to collect completion, and do not reuse old agent names. If that capability is absent, use documented herdr agent commands. Map explore and research to explorer, review and judge to reviewer, and implement, style, and benny to general. Pi-pstack does not own pane or terminal layout.`;
  }
